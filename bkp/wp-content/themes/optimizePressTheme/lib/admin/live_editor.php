    ...
    function parse_shortcode(){
        ...
        if(preg_match('/'.op_shortcode_regex('op_liveeditor_elements').'/s',$sc,$matches) || $op_popup_present){
            ...
            if($GLOBALS['OP_LIVEEDITOR_DEPTH'] === 1){
                ...
                $processed = '<h1>' . __('Content LiveEditor',OP_SN) . '</h1>
                            <div class="epicbox-actual-content">' . $processed .
                                '<a href="#add_element" class="add-new-element"><span>' . __('Add Element', OP_SN) . '</span></a>
                            </div>
...
';
        ...
        $zip->close();
        $out = array('output'=>'<a class="op-download-file" href="'.OP_LIB_URL.'content_layouts/export/'.$tmpdirname.'/'.$filename.'.zip">'.__('Download your layout',OP_SN).'</a> | <a href="#delete" class="delete-file op-delete-file">'.__('Delete File',OP_SN).'</a><input type="hidden" name="zip_filename" id="zip_filename" value="'.$full_dir.'/'.$filename.'.zip" />');
    ...
    function load_content_layouts_preset_dialogs(){
        ...
        add_submenu_page(OP_SN, __('Page Builder',OP_SN), __('Page Builder',OP_SN), 'edit_pages', OP_SN.'-page-builder');
